...
export default function Cart() {
 ...
 useEffect(()=> {
  ...
  <table className=" text-sm text-left mb-2 rtl:text-right text-gray-500 dark:text-gray-400">
    ...
    <tbody>
     {cart?.data.products.map((product)=>  <tr key={product.product.id} className="bg-white border-b bg-white:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 bg-white:hover:bg-gray-600">
        <td className="p-4">
        ...
        <td className="px-3 py-4 font-semibold text-gray-900 dark:text-balck">
          {product.price} EGP
        </td>
        <td className="px-6 py-4">
  ...
  <Link to={'/checkout'} className='bg-main max-w-96 text-center text-white p-3 py-2 rounded-md '>Check Out</Link>
